...
write.xlsx(ged_mat, "../../Output/ged/CoInhib_GED_matrix.xlsx")
ged_mat = read.xlsx("../../Output/ged/CoInhib_GED_matrix.xlsx", sheet = "Sheet 1")
...
g = ggplot(df)+
  geom_point(aes(x=x, y=y, colour=Response, shape=Time), cex = 3) +
  #geom_text_repel(aes(x=x, y=y, colour=Patient, label = PFS_cens)) +
  ggforce::geom_mark_ellipse(aes(x = x, y = y, colour = Patient), linetype="dotted", linewidth=0.25, show.legend = F) +
...
svglite("../../Output/ged/CoInhib_GED_MDS.svg", width = 7, height = 6)
plot(g)
dev.off()

ged_mat = ged_mat[-c(3, 17), -c(3, 17)]
file_name_list = file_name_list[-c(3, 17)]
...
# #### response
# responder = file_name_list[c(1, 2, 3, 4, 8, 9 , 13,
#                              14, 15, 16, 17, 21, 22, 26,
#                              27, 28, 29, 30, 34, 35, 39)]
# d_res = d[c(1, 2, 3, 4, 8, 9 , 13, 14, 15, 16, 17, 21, 22, 26, 27, 28, 29, 30, 34, 35, 39),
#           c(1, 2, 3, 4, 8, 9 , 13, 14, 15, 16, 17, 21, 22, 26, 27, 28, 29, 30, 34, 35, 39)]
# group = factor(c(rep("Baseline", 7), rep("After 1 vac", 7), rep("After 3 vac", 7)))
# res= PerMANOVA.Simple(d_res, group)
# 
# nonresponder = file_name_list[-c(1, 2, 3, 4, 8, 9 , 13,
#                                 14, 15, 16, 17, 21, 22, 26,
#                                 27, 28, 29, 30, 34, 35, 39)]
# d_nonres = d[-c(1, 2, 3, 4, 8, 9 , 13, 14, 15, 16, 17, 21, 22, 26, 27, 28, 29, 30, 34, 35, 39),
#           -c(1, 2, 3, 4, 8, 9 , 13, 14, 15, 16, 17, 21, 22, 26, 27, 28, 29, 30, 34, 35, 39)]
# group_2 = factor(c(rep("Baseline", 6), rep("After 1 vac", 6), rep("After 3 vac", 6)))
# res_2 = PerMANOVA.Simple(d_nonres, group_2)

#### baseline vs after vac 3
patients = file_name_list[c(1:13, 27:39)]
ged = d[c(1:13, 27:39), c(1:13, 27:39)]
ged[c(1:13), c(1:13)] = 0
groups = data.frame(Time = factor(c(rep("Baseline", 13), rep("After 3 vac", 13))),
                    Response = factor(rep(coinhib_ged$Response, 2)))
ged = as.dist(ged)
print(adonis2(ged ~ Time, data = groups))

####
patients_3 = patients[c(14:26)]
ged_3 = d[c(27:39), c(27:39)]
groups_3 = data.frame(Time = factor(c(rep("After 3 vac", 13))),
                    Response = factor(coinhib_ged$Response))
ged_3 = as.dist(ged_3)
print(adonis2(ged_3 ~ Response, data = groups_3, permutations = 1))

#### response
responder = file_name_list[c(1, 2, 3, 4, 8, 9 , 13,
                             27, 28, 29, 30, 34, 35, 39)]
d_res = d[c(1, 2, 3, 4, 8, 9 , 13, 27, 28, 29, 30, 34, 35, 39),
          c(1, 2, 3, 4, 8, 9 , 13, 27, 28, 29, 30, 34, 35, 39)]
d_res[c(1:7), c(1:7)] = 0
group = data.frame(Time=factor(c(rep("Baseline", 7), rep("After 3 vac", 7))) )
d_res = as.dist(d_res)
print(adonis2(d_res ~ Time, data = group))

nonresponder = file_name_list[-c(1, 2, 3, 4, 8, 9 , 13,
                                 14, 15, 16, 17, 21, 22, 26,
                                 27, 28, 29, 30, 34, 35, 39)]
nonresponder = nonresponder[-c(7:12)]
d_nonres = d[-c(1, 2, 3, 4, 8, 9 , 13, 14, 15, 16, 17, 21, 22, 26, 27, 28, 29, 30, 34, 35, 39),
             -c(1, 2, 3, 4, 8, 9 , 13, 14, 15, 16, 17, 21, 22, 26, 27, 28, 29, 30, 34, 35, 39)]
d_nonres = d_nonres[-c(7:12), -c(7:12)]
d_nonres[c(1:6), c(1:6)] = 0
group_2 = data.frame(Time=factor(c(rep("Baseline", 6), rep("After 3 vac", 6))) )
d_nonres = as.dist(d_nonres)
print(adonis2(d_nonres ~ Time, data = group_2))
