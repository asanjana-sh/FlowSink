...
#### This code is only for the DCMyeloid panel, so folder path will always be fixed
#### Folder name will change for the 3 time point data: TP1, TP2, or TP3
folder_path = "C:/Users/AbidaSanjanaShemonti/Documents/R Projects/FlowViz/Data/Raw Data/Bcell_1"
for(folder_name in c("TP1", "TP2", "TP3")){
  ...
  file_list_2 = list.files(path = paste(folder_path, "/", folder_name, sep = ""), pattern = "Bcell.*.fcs", full.names = TRUE)
  ...
  for(file in file_list_1){
    ...
    spill_matrix = read.csv(paste(folder_path, "/", "Bcell_1.csv", sep=""))
  ...
  for(file in file_list_2){
    ...
    spill_matrix = read.csv(paste(folder_path, "/", "Bcell_1_2.csv", sep=""))
    spill_matrix = spill_matrix[, 2:ncol(spill_matrix)]
    colnames(spill_matrix) = marker_channels
