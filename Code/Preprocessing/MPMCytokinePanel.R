...
#### This code is only for the Cytokine panel, so folder path will always be fixed
#### Folder name will change for the 3 time point data: TP1, TP2, or TP3
folder_path = "C:/Users/AbidaSanjanaShemonti/Documents/R Projects/FlowViz/Data/Raw Data/Cytokine"
for(folder_name in c("TP1", "TP2", "TP3")){
  #### Day 1 data file names start with 'Specimen' and Day 2 data file names start with 'cytokine'
  ...
  file_list_2 = list.files(path = paste(folder_path, "/", folder_name, sep = ""), pattern = "cytokine.*.fcs", full.names = TRUE)
  
  #### Day 1, file_list_1, 'Cytokine.csv' compensation matrix file
  for(file in file_list_1){
    ...
    spill_matrix = read.csv(paste(folder_path, "/", "Cytokine.csv", sep=""))
  ...
  #### Day 2, file_list_2, 'Cytokine_2.csv' compensation matrix file
  for(file in file_list_2){
    ...
    spill_matrix = read.csv(paste(folder_path, "/", "Cytokine_2.csv", sep=""))
    spill_matrix = spill_matrix[, 2:ncol(spill_matrix)]
    colnames(spill_matrix) = marker_channels
