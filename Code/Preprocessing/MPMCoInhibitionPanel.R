...
#### This code is only for the Co-inhibition panel, so folder path will always be fixed
...
#### Day 1 data file names start with 'Specimen' and Day 2 data file names start with 'co-inhib'
...
file_list_2 = list.files(path = paste(folder_path, "/", folder_name, sep = ""), pattern = "co-inhib.*.fcs", full.names = TRUE)

#### Day 1, file_list_1, 'Co-inhibition.csv' compensation matrix file
...
#### Day 2, file_list_2, 'Co-inhibition_2.csv' compensation matrix file
for(file in file_list_2){
  ...
  spill_matrix = read.csv(paste(folder_path, "/", "Co-inhibition_2.csv", sep=""))
  spill_matrix = spill_matrix[, 2:ncol(spill_matrix)]
  colnames(spill_matrix) = marker_channels
