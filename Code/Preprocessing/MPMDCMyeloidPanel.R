...
for(folder_name in c("TP1", "TP2", "TP3")){
  ...
  file_list_2 = list.files(path = paste(folder_path, "/", folder_name, sep = ""), pattern = "DC.*.fcs", full.names = TRUE)
  ...
  for(file in file_list_1){
    ...
    spill_matrix = read.csv(paste(folder_path, "/", "DCMyeloid.csv", sep=""))
  ...
  for(file in file_list_2){
    ...
    spill_matrix = read.csv(paste(folder_path, "/", "DCMyeloid_2.csv", sep=""))
    spill_matrix = spill_matrix[, 2:ncol(spill_matrix)]
    colnames(spill_matrix) = marker_channels
